...
* **Atribuições:** Vai criar as estruturas de estado (palavra a ser adivinhada, letras que já foram usadas, quantidade de erros). Implementar a atualização do estado da palavra (se uma letra já foi acertada, retornar a lista de letras escolhidas que não fazem parte da palavra secreta). Implementar a função de inicializar o jogo, processar a jogada com a letra enviada e fazer a contagem de jogadas (passou de 5 erros, perdeu o jogo). Lida também com a lógica de pontuação (vitória em single-player conta como peso 1 para a pontuação do jogador; vitória multiplayer conta peso 2 para os pontos)

### Taygo: Interface Visual (Visão / GUI)
**Arquivo de trabalho principal:** `main.py`
